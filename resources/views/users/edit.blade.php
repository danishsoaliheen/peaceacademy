...
<div class="user-hero d-flex justify-content-between align-items-center flex-wrap gap-3">
    <div><h2 class="mb-1" style="font-size:1.25rem;font-weight:700;"><i class="fas fa-user-edit me-2"></i>Edit User</h2><p class="mb-0" style="opacity:.68;font-size:.82rem;">Update account details, role or password</p></div>
    ...
    <div class="col-lg-8">
        <div class="section-card card">
            ...
            <div class="card-body"><div class="row g-3">
                <div class="col-md-6"><label class="form-label">Name <span class="text-danger">*</span></label><input name="name" value="{{ old('name',$user->name) }}" class="form-control @error('name') is-invalid @enderror" required>@error('name')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
                ...
                <div class="col-md-6"><label class="form-label">New Password</label><input type="password" name="password" class="form-control" minlength="8"><small class="text-muted">Leave blank to keep the current password.</small></div>
                <div class="col-md-6"><label class="form-label">Confirm New Password</label><input type="password" name="password_confirmation" class="form-control" minlength="8"></div>
            </div></div>
        </div>
        <div class="section-card card">
            <div class="card-header"><span class="s-icon" style="background:#64748b;width:27px;height:27px;border-radius:6px;display:inline-flex;align-items:center;justify-content:center;color:#fff;"><i class="fas fa-circle-info"></i></span><strong>Account Status</strong></div>
            <div class="card-body d-flex justify-content-between align-items-center">
                <div><div class="fw-bold">{{ $user->is_active ? 'Active account' : 'Inactive account' }}</div><small class="text-muted">{{ $user->is_active ? 'This user can log in.' : 'This user is blocked from logging in.' }}</small></div>
                <span class="pa-badge {{ $user->is_active ? 'pa-badge-green' : 'pa-badge-red' }}">{{ $user->is_active ? 'Active' : 'Inactive' }}</span>
            </div>
        </div>
    </div>
    <div class="col-lg-4">
        <div class="section-card card">
            ...
            <div class="card-body">
                ...
                <select name="role" class="form-select" required>
                    @foreach($roles as $key => $label)<option value="{{ $key }}" @selected(old('role',$user->role) === $key)>{{ $label }}</option>@endforeach
                </select>
            </div>
        </div>
        <div class="d-grid gap-2"><button class="btn-save"><i class="fas fa-save me-1"></i> Save Changes</button><a href="{{ route('users.index') }}" class="btn btn-light" style="border-radius:9px;">Cancel</a></div>
    </div>
</div>
</form>
@endsection